        ...
        for j in range(5):
            ...
            for z in range(N_Cls):
                y[s * i:s * i + s, s * j:s * j + s, z] = generate_mask_for_image_and_class(
                    (img.shape[2], img.shape[1]), id, z + 1)[:s, :s]
...
def train_net(img, msk, x_val, y_val):
    ...
    for i in range(1):
        ...
        model.fit(x_trn, y_trn, batch_size=32, nb_epoch=1, verbose=1, shuffle=True,
...
def predict_id(id, model, trs):
    ...
    # cnv = np.zeros((ISZ*6, ISZ*6, 3)).astype(np.float32)
    prd = np.zeros((N_Cls, ISZ*6, ISZ*6)).astype(np.float32)
    # cnv[:img.shape[2], :img.shape[1], :] = x.transpose((1, 2, 0))
    cnv = x.transpose((1, 2, 0))
